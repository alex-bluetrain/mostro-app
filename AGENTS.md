...
- Current user: `${MOSTRO_URL}/users/me`   (NOT `/api/me`, NOT `/me`, NOT `/api/users/me`)
- Chat OpenUI:  `${MOSTRO_URL}/agents/mostro-supervisor/openui`   (NOT `/api/agents/...`)

Before writing any route, verify against `registerApiRoute()` calls in
`mostro/src/mastra/routes/`. Do not assume `/api` — it 404s here.
